  ...
  User.find({
    ...
    .then(async userData => {
      if (userData.length === 0) {
        ...
        await verifyPwd.passwordMatchVerification(
          userInfo.password,
          userData[0].password
        )
      ) {
        const token = await accessTokenDao.generateToken(
          userData[0].email,
          userData[0]._id
        );
        const accessTokenValue = await accessTokenDao.saveTokenAndGetAccessToken(
          token,
          userData[0]._id
        );
        const basedAccesstoken = await accessTokenDao.generateToken(
          accessTokenValue,
          userData[0]._id
        );
